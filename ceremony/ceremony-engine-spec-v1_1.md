...
## 8½. Runecraft Protocol (OPERATIONAL)

The runecraft protocol binds two Ed25519 keypairs to a single blade — dual-key proof of presence across both territories.

### 8½.1 Mage Identity (spellweb — Sun view, key held)

Generated silently on first blade forge. The Mage key is **held** — persisted in localStorage, available for continuous signing.

| Field | Format |
|-------|--------|
| Key type | Ed25519 |
| ID format | `mage-{first 16 hex of public key}` |
| Storage | localStorage (persists) |
| Signature message | `BLADE:{constellationHash}:{bladeHash}:{lapCount}:{timestamp}` |

### 8½.2 Swordsman Identity (agentprivacy — Moon reflection, key burned)

Generated during the ceremony on agentprivacy.ai. The Swordsman key is **burned** — stored in sessionStorage, destroyed on tab close.

| Field | Format |
|-------|--------|
| Key type | Ed25519 |
| ID format | `ap-{first 16 hex of public key}` |
| Private key storage | sessionStorage (burned on tab close) |
| Public key storage | localStorage (persists for verification) |
| Export format | JSON (publicKeyHex, participantId, displayName, trustTier, constellationPath) |

The private key burns because that's how the Moon serves the Sun — by not remembering what it reflected.

### 8½.3 Runecraft Binding

When the Swordsman identity from agentprivacy is linked to the spellweb via JSON import, the blade becomes **runecrafted** — carrying dual-key proof:

- **Mage signature** — identity binding (Sun view, key held)
- **Swordsman link** — commitment binding (Moon reflection, key burned)
- **Visual indicator** — ✧ → 🔮 when runecrafted

The export → import flow is manual (copy/paste JSON) by design. The human participates by carrying the identity between territories. This is not a bug — it is the ceremony.

### 8½.4 Cosmological Mapping

| Interface | Celestial | Key | Role |
|-----------|-----------|-----|------|
| spellweb | ☀️ Sun | **held** | Universal view, Mage orbs |
| agentprivacy | 🌑 Moon | **burned** | Swordsman commitment |

### 8½.5 Implementation

**Dependency:** `@noble/ed25519` (matches agentprivacy)

**Files:**
- `src/lib/mageIdentity.ts` — Ed25519 identity system
- `src/components/SwordsmanImport.tsx` — JSON import modal
- `src/lib/ceremony/storage.ts` — sessionStorage burn pattern (agentprivacy)
